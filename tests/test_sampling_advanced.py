...
class TestSamplingEdgeCases:
    ...
    def test_empty_arrays(self):
        """Test behavior with empty/minimal arrays."""
        basis = pylibsparseir.FiniteTempBasis('F', 1.0, 10.0, 1e-6)
        
        # Test with single point
        single_point = np.array([0.5])
        smpl = pylibsparseir.TauSampling(basis, single_point)
        
        assert len(smpl.tau) == 1
        assert smpl.tau[0] == 0.5
        
        # evaluate/fit with single point should work but may not be well-conditioned
        Gl = np.zeros(basis.size)
        Gl[0] = 1.0
        ...
        try:
            ...
            # Fitting from single point may not work well, but shouldn't crash
            Gl_recovered = smpl.fit(Gtau)
            assert len(Gl_recovered) == basis.size
            
        except RuntimeError:
            # Single point sampling may be ill-conditioned - this is expected
            pass
